...
"@
                    ...
                    if ($Script:PromptOutputTextBox -ne $null) {
                        ...
                        # フォールバック: 基本レポート生成
                        Write-GuiLog "⚠️ テンプレートが見つかりません。フォールバックレポートを生成します。" "WARNING"
                        
                        # フォールバック処理
                        Set-GuiProgress -Value 100 -Status "完了"
                        try { Write-GuiLog "フォールバックレポートを生成しました" "WARNING" -ShowNotification } catch { }
                        if ($Script:PromptOutputTextBox -ne $null) {
                            $Script:PromptOutputTextBox.AppendText("⚠️ テンプレートが見つかりません: $reportName`r`n")
                            $Script:PromptOutputTextBox.AppendText("💡 フォールバック処理で基本レポートを作成しました`r`n`r`n")
                            $Script:PromptOutputTextBox.ScrollToCaret()
                        }
